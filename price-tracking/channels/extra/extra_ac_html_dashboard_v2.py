...
FREESTANDING_KEYWORDS = ['FLOOR STANDING', 'FREE STANDING', 'FREESTANDING', 'FLOOR-STANDING', 'FREESTAND']
...
function renderSkuStatus(){
  ...
  function skuCard(rec,ab,gb,cfg){
    ...
    const extraUrl=rec.s?`https://www.extra.com/en-sa/p/${rec.s}`:'';
    const nameHtml=rec.url||rec.u||extraUrl?`<a href="${rec.url||rec.u||extraUrl}" target="_blank" class="text-blue-600 hover:underline">${rec.n||rec.s}</a>`:(rec.n||rec.s);
