...
function renderSkuStatus(){
  ...
  function skuCard(rec,ab,gb,cfg){
    ...
    const almanUrl=rec.u?`https://www.almanea.sa/en/product/${rec.u}`:'';
    const nameHtml=rec.url||almanUrl?`<a href="${rec.url||almanUrl}" target="_blank" class="text-blue-600 hover:underline">${rec.n||rec.s}</a>`:(rec.n||rec.s);
