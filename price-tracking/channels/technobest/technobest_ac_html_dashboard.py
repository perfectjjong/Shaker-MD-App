#!/usr/bin/env python3
"""
TechnoBest AC Price Tracking - HTML Dashboard
Sections: KPIs, Price Alerts, New/Disc, Category KPI, Brand Compare, Price Trend, Full SKU, Stock + Promotion
Based on Bin Momen template, adapted for TechnoBest data structure.
"""
...
INPUT_FILE  = os.path.join(CURRENT_DIR, "TechnoBest_AC_Master.xlsx")
OUTPUT_FILE = os.path.join(CURRENT_DIR, "technobest_ac_dashboard.html")
...
# Numeric coercion
for col in ['Ton', 'regular_price', 'sale_price', 'BTU', 'discount_pct']:
    if col in df.columns:
        df[col] = pd.to_numeric(df[col], errors='coerce')

# Discount rate: discount_pct is 0-100, convert to 0-1
df['Discount_Rate'] = df['discount_pct'] / 100.0

# Fill missing discount rate from prices
missing_dr = df['Discount_Rate'].isna()
valid = missing_dr & df['regular_price'].notna() & df['sale_price'].notna() & (df['regular_price'] > 0)
df.loc[valid, 'Discount_Rate'] = 1 - df.loc[valid, 'sale_price'] / df.loc[valid, 'regular_price']

# In_Stock boolean (is_available=True and is_out_of_stock=False)
df['_in_stock'] = df['is_available'].fillna(False) & ~df['is_out_of_stock'].fillna(True)
...
for _, r in df.iterrows():
    records.append({
        ...
        'ba': safe(r.get('brand_ar')) if pd.notna(r.get('brand_ar')) else '',
        ...
        't': safe(r.get('Ton')),
        'sp': safe(r.get('regular_price')),
        'sl': safe(r.get('sale_price')),
        'dr': safe(r.get('Discount_Rate')),
        'fp': safe(r.get('sale_price')),  # effective = sale_price
        'ins': bool(r.get('_in_stock')) if pd.notna(r.get('_in_stock')) else False,
        'onsale': bool(r.get('is_on_sale')) if pd.notna(r.get('is_on_sale')) else False,
        'oos': bool(r.get('is_out_of_stock')) if pd.notna(r.get('is_out_of_stock')) else False,
        'url': str(r.get('url','')) if pd.notna(r.get('url')) else '',
        'btu': safe(r.get('BTU')),
        'promo': str(r.get('promotion','')) if pd.notna(r.get('promotion')) else '',
        'sub': str(r.get('subtitle','')) if pd.notna(r.get('subtitle')) else '',
...
# Promotion types for analysis
promo_types = sorted(df[df['promotion'].str.len() > 0]['promotion'].unique().tolist())
...
BRAND_COLORS = {b: c for b, c in zip(brands_list, [
    ...
    '#DC143C','#8B4513','#20B2AA','#FF6347','#4682B4','#DAA520','#556B2F','#CD5C5C',
    '#483D8B'])}
...
<title>TechnoBest AC Price Tracker</title>
...
.type-cat{background:#2563eb;color:#fff}.type-ton{background:#e5e7eb;color:#374151}
...
.promo-badge{display:inline-block;padding:1px 6px;border-radius:4px;font-size:9px;font-weight:700;background:#fef3c7;color:#92400e;letter-spacing:.3px}
...
<header class="bg-gradient-to-r from-navy-900 to-navy-700 text-white shadow-lg">
  <div class="max-w-[1600px] mx-auto px-4 sm:px-6 py-3 sm:py-4 flex flex-wrap items-center justify-between gap-2 sm:gap-4">
    <div><h1 class="text-base sm:text-xl font-bold tracking-wide">TechnoBest AC Price Tracker</h1>
...
<div class="sticky top-0 z-40 bg-white/95 backdrop-blur border-b border-gray-200 shadow-sm no-print">
  <div class="max-w-[1600px] mx-auto px-3 sm:px-6 py-2 sm:py-2.5 flex flex-wrap items-center gap-2">
    ...
    <div id="gf_cat"></div><div id="gf_ton"></div><div id="gf_brand"></div>
    <div class="w-px h-5 bg-gray-300"></div>
...
<nav class="max-w-[1600px] mx-auto px-3 sm:px-6 pt-2 sm:pt-3 no-print">
  <div class="flex flex-wrap gap-1.5 text-[11px]">
    ...
    <a href="#sec-stock" class="px-3 py-1 bg-white text-navy-800 rounded-full border border-gray-200 hover:bg-navy-50">Stock &amp; Promo</a>
  </div>
</nav>
...
<section id="sec-catKPI" class="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
  ...
  <div class="flex flex-wrap items-center gap-2 mb-3 pb-2 border-b border-gray-100">
    ...
    <div id="s4_cat"></div><div id="s4_ton"></div><div id="s4_brand"></div>
...
<section id="sec-brand" class="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
  ...
  <div class="flex flex-wrap items-center gap-2 mb-3 pb-2 border-b border-gray-100">
    <span class="text-[10px] font-bold text-gray-400 uppercase">Filters</span>
    <div id="s5_cat"></div><div id="s5_ton"></div><div id="s5_brand"></div>
    <div class="w-px h-5 bg-gray-200"></div>
...
<section id="sec-trend" class="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
  ...
  <div class="flex flex-wrap items-center gap-2 mb-3 pb-2 border-b border-gray-100">
    <span class="text-[10px] font-bold text-gray-400 uppercase">Filters</span>
    <div id="s6_cat"></div><div id="s6_ton"></div><div id="s6_brand"></div>
    <div class="w-px h-5 bg-gray-200"></div>
...
<section id="sec-sku" class="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
  ...
  <div class="flex flex-wrap items-center gap-2 mb-3 pb-2 border-b border-gray-100">
    <span class="text-[10px] font-bold text-gray-400 uppercase">Filters</span>
    <div id="s7_cat"></div><div id="s7_ton"></div><div id="s7_brand"></div>
    <div class="w-px h-5 bg-gray-200"></div>
    ...
    <div class="flex rounded-lg overflow-hidden border border-gray-200">
      ...
      <button type="button" class="stock-btn px-2 py-1 text-[10px] font-medium bg-gray-50 text-red-600" data-val="out">Out</button>
      <button type="button" class="stock-btn px-2 py-1 text-[10px] font-medium bg-gray-50 text-amber-600" data-val="promo">Promo</button></div>
...
<!-- SEC 8: Stock & Promotion Dashboard -->
<section id="sec-stock" class="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
  <h2 class="text-sm font-bold text-navy-800 border-b-2 border-navy-800 pb-2 mb-3">Stock &amp; Promotion Dashboard</h2>
  <div class="filter-bar flex flex-wrap items-center gap-1.5 mb-3">
    <span class="text-[10px] font-bold text-gray-400 uppercase">Filters</span>
    <div id="s8_date"></div>
    <div id="s8_cat"></div><div id="s8_ton"></div><div id="s8_brand"></div>
    <div class="w-px h-5 bg-gray-200"></div>
  ...
  <!-- Stock KPIs -->
  <div id="stockKpiGrid" class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5 mb-4"></div>
  ...
  <!-- Promotion Analysis -->
  <h3 class="text-xs font-bold text-gray-500 uppercase mt-4 mb-2">Promotion Analysis</h3>
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
    <div>
      <h4 class="text-[11px] font-semibold text-gray-600 mb-2">Promotion Type Distribution</h4>
      <div style="height:320px"><canvas id="promoTypeChart"></canvas></div>
    </div>
    <div>
      <h4 class="text-[11px] font-semibold text-gray-600 mb-2">Promo vs Non-Promo Price Impact</h4>
      <div style="height:320px"><canvas id="promoImpactChart"></canvas></div>
    </div>
  </div>

  <!-- Promo SKU List -->
  <h3 class="text-xs font-bold text-gray-500 uppercase mt-2 mb-2">Promotion SKU List</h3>
  <div class="tbl-wrap mb-4" style="max-height:350px;overflow-y:auto"><table id="tblPromo"><thead></thead><tbody></tbody></table></div>

  <!-- Brand Charts -->
  <h3 class="text-xs font-bold text-gray-500 uppercase mt-2 mb-2">Brand Availability & Promotion</h3>
...
<footer class="text-center py-3 text-[10px] text-gray-400">TechnoBest AC Price Tracking Dashboard &middot; Generated GENERATED_AT</footer>
"""
...
const searchMatch=(r,q)=>!q||((r.b||'')+(r.s||'')+(r.m||'')+(r.n||'')+(r.c||'')+(r.t||'')+(r.promo||'')).toLowerCase().includes(q);
...
function resetF(f){f.cat.reset();f.ton.reset();f.brand.reset();}
...
let gfDate,s8Date;
const ST={alertDir:'all',s4q:'',s5q:'',s6q:'',s7q:'',s8q:'',skuStock:'all',s5agg:'avg',s6agg:'avg'};
...
function resetStock(){s8Date.reset();resetF(S8F);ST.s8q='';document.getElementById('s8_search').value='';renderStock();}
...
function renderKPIs(lat,prev){
  ...
  const promoCount=lat.filter(r=>r.promo&&r.promo.length>0).length;
  const cards=[
    ...
    {v:promoCount,l:'With Promo',c:'border-l-purple-500 bg-purple-50',vc:'text-purple-700'},
  ];
...
// ═══ SEC 2: ALERTS ══════════════════════════════════════════════════════════
const AC=[{k:'b',l:'Brand'},{k:'s',l:'Product'},{k:'m',l:'SKU'},{k:'c',l:'Category'},{k:'ton',l:'Ton'},{k:'prev',l:'Prev',f:fmtSAR},{k:'curr',l:'Curr',f:fmtSAR},{k:'chg',l:'Change',f:fmtChg},{k:'chgPct',l:'Chg%',f:fmtPctR},{k:'promo',l:'Promotion'}];
function renderAlerts(){
  ...
  Object.keys(lm).filter(s=>s in pm).forEach(s=>{
    ...
    rows.push({b:rn.b,s:rn.n,m:rn.m,c:rn.c,ton:rn.t!=null?rn.t.toFixed(1)+'T':'-',prev:po,curr:pn,chg,chgPct:po?chg/po*100:0,url:rn.url,promo:rn.promo||'-'});
  });
  ...
  tbl.querySelector('tbody').innerHTML=rows.length?rows.map(r=>'<tr>'+AC.map(c=>{let v=c.f?c.f(r[c.k]):(r[c.k]??'-');let cls='';if((c.k==='chg'||c.k==='chgPct')&&r.chg!=null)cls=r.chg>0?'up-cell':'dn-cell';if(c.k==='s'&&r.url)v=`<a href="${r.url}" target="_blank" class="text-blue-600 hover:underline">${v}</a>`;if(c.k==='promo'&&v&&v!=='-')v=`<span class="promo-badge">${v}</span>`;return`<td class="${cls}">${v}</td>`;}).join('')+'</tr>').join(''):'<tr><td colspan="10" class="text-center text-gray-400 py-6">No changes</td></tr>';
}
...
function renderNewDisc(lat,prev){
  ...
  const card=(r,clr)=>`<div class="border border-${clr}-200 bg-${clr}-50 rounded-lg p-2.5 flex justify-between items-center"><div><span class="text-xs font-bold" style="color:${colorOf(r.b)}">${r.b}</span> <span class="text-[10px] text-gray-400">${r.m||''}</span><div class="text-[11px] mt-0.5 truncate max-w-[280px]">${r.url?`<a href="${r.url}" target="_blank" class="text-blue-600 hover:underline">${r.n}</a>`:r.n}</div><div class="text-[10px] text-gray-400">${r.c||''} &middot; ${r.t?r.t.toFixed(1)+'T':''} ${r.promo?'&middot; <span class="promo-badge">'+r.promo+'</span>':''}</div></div><div class="text-sm font-bold text-gray-700">${fmtSAR(r.fp)} SAR</div></div>`;
...
// ═══ SEC 4: CATEGORY KPI ════════════════════════════════════════════════════
const CK=[{l:'Type',k:'type'},{l:'Segment',k:'label'},{l:'SKUs',k:'cnt'},{l:'Avg Sale',k:'avgP',f:fmtSAR},{l:'Avg Original',k:'avgStd',f:fmtSAR},{l:'Avg Disc %',k:'avgDisc',f:fmtPctR},{l:'Available',k:'inStk'},{l:'Promo',k:'promoCnt'},{l:'LG Avg',k:'lgAvg',f:fmtSAR},{l:'LG Gap',k:'lgGap',f:fmtChg},{l:'LG Gap %',k:'lgGapPct',f:fmtPctR}];
const TYPE_BADGE={cat:'<span class="type-badge type-cat">Category</span>',ton:'<span class="type-badge type-ton">Ton</span>'};
...
function segKPI(data){
  ...
  const promoCnt=data.filter(r=>r.promo&&r.promo.length>0).length;
  ...
  return{cnt,avgP,avgStd,avgDisc,inStk,promoCnt,lgAvg,lgGap,lgGapPct};
}
...
function renderS4(){
  ...
  cats.forEach(cat=>{
    ...
    const tons=[...new Set(catD.map(r=>r.t))].filter(Boolean).sort((a,b)=>a-b);
    tons.forEach(t=>{
      const tD=catD.filter(r=>r.t===t);if(!tD.length)return;
...
function renderS5(){
  ...
  cats.forEach(cat=>{const cD=lat.filter(r=>r.c===cat);
    [...new Set(cD.map(r=>r.t))].filter(x=>x!=null&&!isNaN(x)).sort((a,b)=>a-b).forEach(t=>{const tD=cD.filter(r=>r.t===t);if(!tD.length)return;
      const seg=(cat||'')+' / '+Number(t).toFixed(1)+'T';
...
// ═══ SEC 7: FULL SKU ═══════════════════════════════════════════════════════
const SK=[{k:'b',l:'Brand'},{k:'n',l:'Product'},{k:'m',l:'SKU'},{k:'c',l:'Category'},{k:'t',l:'Ton',f:v=>v!=null?v.toFixed(1)+'T':'-'},{k:'sp',l:'Original',f:fmtSAR},{k:'fp',l:'Sale',f:fmtSAR},{k:'dr',l:'Disc%',f:fmtPct},{k:'ins',l:'Avail',f:v=>v?'Yes':'No'},{k:'promo',l:'Promotion'},{k:'sub',l:'Subtitle'}];
function renderS7(){
  ...
  if(ST.skuStock==='promo')lat=lat.filter(r=>r.promo&&r.promo.length>0);
  ...
  tbl.querySelector('tbody').innerHTML=lat.length?lat.map(r=>'<tr>'+SK.map(c=>{let v=c.f?c.f(r[c.k]):(r[c.k]??'-');if(c.k==='n'&&r.url)v=`<a href="${r.url}" target="_blank" class="text-blue-600 hover:underline">${v}</a>`;if(c.k==='ins')v=r.ins?'<span class="stk-badge stk-ok">Yes</span>':'<span class="stk-badge stk-out">No</span>';if(c.k==='promo'&&v&&v!=='-'&&v!=='')v=`<span class="promo-badge">${v}</span>`;return`<td>${v}</td>`;}).join('')+'</tr>').join(''):'<tr><td colspan="11" class="text-center text-gray-400 py-6">No data</td></tr>';
}
...
function downloadExcel(){
  ...
  if(ST.skuStock==='out')lat=lat.filter(r=>!r.ins);
  if(ST.skuStock==='promo')lat=lat.filter(r=>r.promo&&r.promo.length>0);
  const rows=lat.map(r=>({Brand:r.b,Product:r.n,SKU:r.m,Category:r.c,Ton:r.t,Original:r.sp,Sale:r.fp,Discount:r.dr!=null?(r.dr*100).toFixed(1)+'%':'',Available:r.ins?'Yes':'No',Promotion:r.promo,Subtitle:r.sub,URL:r.url}));
  const ws=XLSX.utils.json_to_sheet(rows);const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,ws,'SKUs');
  XLSX.writeFile(wb,'TechnoBest_AC_'+cur+'.xlsx');
}

// ═══ SEC 8: STOCK & PROMOTION ══════════════════════════════════════════════
let stockTrendChart=null,stockBrandBarChart=null,stockCatDonutChart=null,promoTypeChartObj=null,promoImpactChartObj=null;
...
function renderStock(){
  ...
  document.getElementById('s8_compare').textContent='Data as of: '+latDate;

  // KPIs
  const total=lat.length;
  const available=lat.filter(r=>r.ins).length;
  const unavailable=lat.filter(r=>!r.ins).length;
  const onSale=lat.filter(r=>r.onsale).length;
  const promoCount=lat.filter(r=>r.promo&&r.promo.length>0).length;
  const avgDisc=lat.filter(r=>r.dr!=null);
  const avgDiscVal=avgDisc.length?Math.round(mean(avgDisc.map(r=>r.dr*100))*10)/10:0;

  const sCards=[
    {v:total,l:'Total SKUs',c:'border-l-navy-800 bg-navy-50',vc:'text-navy-800'},
    {v:available,l:'Available',c:'border-l-green-500 bg-green-50',vc:'text-green-600'},
    {v:unavailable,l:'Unavailable',c:'border-l-red-500 bg-red-50',vc:'text-red-600'},
    {v:onSale,l:'On Sale',c:'border-l-blue-500 bg-blue-50',vc:'text-blue-600'},
    {v:promoCount,l:'With Promotion',c:'border-l-amber-500 bg-amber-50',vc:'text-amber-600'},
    {v:avgDiscVal.toFixed(1)+'%',l:'Avg Discount',c:'border-l-purple-500 bg-purple-50',vc:'text-purple-600'},
  ];
  ...
  // Availability Trend
  const trendAvail=DATES.map(d=>{const dd=applyF(DATA.filter(r=>r.d===d),S8F);return dd.filter(r=>r.ins).length;});
  ...
  stockTrendChart=new Chart(ctx1,{type:'line',data:{labels:DATES,datasets:[
    {label:'Available',data:trendAvail,borderColor:'#16a34a',backgroundColor:'rgba(22,163,74,.1)',borderWidth:2,pointRadius:3,tension:.3,fill:true},
    {label:'Total',data:trendTotal,borderColor:'#2E75B6',backgroundColor:'rgba(46,117,182,.05)',borderWidth:2,pointRadius:3,tension:.3,fill:false,borderDash:[5,3]}
  ]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom',labels:{font:{size:10}}},datalabels:{display:false}},scales:{x:{ticks:{font:{size:10},maxRotation:45}},y:{ticks:{callback:v=>v.toLocaleString()}}}}});

  // Promotion Analysis - Type Distribution
  const promoTypes={};lat.forEach(r=>{if(r.promo&&r.promo.length>0){promoTypes[r.promo]=(promoTypes[r.promo]||0)+1;}});
  const ptItems=Object.entries(promoTypes).sort((a,b)=>b[1]-a[1]);
  const promoColors=['#f59e0b','#3b82f6','#10b981','#ef4444','#8b5cf6','#ec4899','#14b8a6','#f97316'];
  const ctx4=document.getElementById('promoTypeChart').getContext('2d');
  if(promoTypeChartObj)promoTypeChartObj.destroy();
  promoTypeChartObj=new Chart(ctx4,{type:'doughnut',data:{labels:ptItems.map(x=>x[0]),datasets:[{data:ptItems.map(x=>x[1]),backgroundColor:promoColors.slice(0,ptItems.length)}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom',labels:{font:{size:9},boxWidth:8}},datalabels:{display:true,color:'#fff',font:{size:10,weight:'bold'},formatter:(v,ctx)=>{const sum=ctx.dataset.data.reduce((a,b)=>a+b,0);return sum?(v/sum*100).toFixed(0)+'%':'';}}}}});

  // Promo vs Non-Promo Price Impact
  const promoSKUs=lat.filter(r=>r.promo&&r.promo.length>0&&r.fp!=null);
  const nonPromoSKUs=lat.filter(r=>(!r.promo||r.promo.length===0)&&r.fp!=null);
  const promoAvgPrice=promoSKUs.length?Math.round(mean(promoSKUs.map(r=>r.fp))):0;
  const nonPromoAvgPrice=nonPromoSKUs.length?Math.round(mean(nonPromoSKUs.map(r=>r.fp))):0;
  const promoAvgDisc=promoSKUs.filter(r=>r.dr!=null).length?Math.round(mean(promoSKUs.filter(r=>r.dr!=null).map(r=>r.dr*100))*10)/10:0;
  const nonPromoAvgDisc=nonPromoSKUs.filter(r=>r.dr!=null).length?Math.round(mean(nonPromoSKUs.filter(r=>r.dr!=null).map(r=>r.dr*100))*10)/10:0;

  const ctx5=document.getElementById('promoImpactChart').getContext('2d');
  if(promoImpactChartObj)promoImpactChartObj.destroy();
  promoImpactChartObj=new Chart(ctx5,{type:'bar',data:{labels:['Avg Sale Price (SAR)','Avg Discount (%)','SKU Count'],datasets:[
    {label:'With Promotion',data:[promoAvgPrice,promoAvgDisc,promoSKUs.length],backgroundColor:'#f59e0b'},
    {label:'Without Promotion',data:[nonPromoAvgPrice,nonPromoAvgDisc,nonPromoSKUs.length],backgroundColor:'#6b7280'}
  ]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom',labels:{font:{size:10}}},datalabels:{display:true,anchor:'end',align:'top',font:{size:10,weight:'bold'},formatter:v=>v.toLocaleString()}},scales:{y:{ticks:{callback:v=>v.toLocaleString()}}}}});

  // Promo SKU table
  const promoRows=lat.filter(r=>r.promo&&r.promo.length>0);
  const promoCols=[{k:'b',l:'Brand'},{k:'n',l:'Product'},{k:'m',l:'SKU'},{k:'c',l:'Category'},{k:'t',l:'Ton',f:v=>v!=null?v.toFixed(1)+'T':'-'},{k:'sp',l:'Original',f:fmtSAR},{k:'fp',l:'Sale',f:fmtSAR},{k:'dr',l:'Disc%',f:fmtPct},{k:'promo',l:'Promotion'}];
  const ptbl=document.getElementById('tblPromo');
  ptbl.querySelector('thead').innerHTML='<tr>'+promoCols.map((c,i)=>`<th onclick="sortTbl('tblPromo',${i})">${c.l}</th>`).join('')+'</tr>';
  ptbl.querySelector('tbody').innerHTML=promoRows.length?promoRows.map(r=>'<tr>'+promoCols.map(c=>{let v=c.f?c.f(r[c.k]):(r[c.k]??'-');if(c.k==='n'&&r.url)v=`<a href="${r.url}" target="_blank" class="text-blue-600 hover:underline">${v}</a>`;if(c.k==='promo'&&v&&v!=='-')v=`<span class="promo-badge">${v}</span>`;return`<td>${v}</td>`;}).join('')+'</tr>').join(''):'<tr><td colspan="9" class="text-center text-gray-400 py-4">No promotions</td></tr>';

  // Brand availability bar chart
  const brandAvail={};lat.forEach(r=>{if(!brandAvail[r.b])brandAvail[r.b]={avail:0,total:0};brandAvail[r.b].total++;if(r.ins)brandAvail[r.b].avail++;});
  const bItems=Object.entries(brandAvail).sort((a,b)=>b[1].total-a[1].total);
  ...
  stockBrandBarChart=new Chart(ctx2,{type:'bar',data:{labels:bItems.map(x=>x[0]),datasets:[
    {label:'Available',data:bItems.map(x=>x[1].avail),backgroundColor:'#16a34a'},
    {label:'Unavailable',data:bItems.map(x=>x[1].total-x[1].avail),backgroundColor:'#dc2626'}
  ]},options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom',labels:{font:{size:10}}},datalabels:{display:false}},scales:{x:{stacked:true,ticks:{callback:v=>v.toLocaleString()}},y:{stacked:true,ticks:{font:{size:10}}}}}});

  // Category donut
  const catCount={};lat.forEach(r=>{catCount[r.c]=(catCount[r.c]||0)+1;});
  const cItems=Object.entries(catCount).sort((a,b)=>b[1]-a[1]);
...
# ── GitHub Pages Deploy ────────────────────────────────────────────────────────
DEPLOY_DIR = os.path.join(os.path.expanduser("~"), "tmp_deploy_technobest_dashboard")
if os.path.isdir(os.path.join(DEPLOY_DIR, ".git")):
    ...
    if result.returncode != 0:
        ...
        result = subprocess.run(["git", "push"], cwd=DEPLOY_DIR)
    if result.returncode == 0:
        print("\n🚀 Deployed: https://perfectjjong.github.io/technobest-ac-dashboard/")
    else:
...
CLOUDFLARE_DEST = os.path.join(SHAKER_DIR, "docs", "dashboards", "technobest-price")
if os.path.exists(os.path.join(SHAKER_DIR, ".git")):
    ...
    try:
        subprocess.run(["git", "add", "docs/dashboards/technobest-price/index.html"],
                       cwd=SHAKER_DIR, check=True, capture_output=True)
        result = subprocess.run(
            ["git", "commit", "-m",
             f"Update Techno Best Price dashboard ({generated_at})"],
