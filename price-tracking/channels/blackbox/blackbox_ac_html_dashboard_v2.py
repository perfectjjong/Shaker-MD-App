#!/usr/bin/env python3
"""
Black Box AC Price Tracking - HTML Dashboard V2
All filters multi-select, section-level filters, Excel download, Stock dashboard.
Track B (Stock 있는 채널) - Al Manea 패턴 기반, Black Box 고유 필드 반영.
"""
...
INPUT_FILE  = os.path.join(CURRENT_DIR, "Black Box_AC_Price tracking_Master.xlsx")
OUTPUT_FILE = os.path.join(CURRENT_DIR, "blackbox_ac_dashboard_v2.html")
...
df = pd.read_excel(INPUT_FILE, sheet_name='Product_DB')
print(f"      {len(df):,} rows, columns: {list(df.columns)}")

# Parse Scraped At → date_only
df['Scraped At'] = pd.to_datetime(df['Scraped At'])
df['date_only'] = df['Scraped At'].dt.date

# Numeric coercion
for col in ['Ton','Original Price','Sale Price','Effective Price','BP Price','Effective BP',
            'Discount %','Extra Disc %','Stock Qty','BTU','Install SAR']:
    if col in df.columns:
        df[col] = pd.to_numeric(df[col], errors='coerce')

# Discount % is already a float like 48.7 → convert to 0~1 ratio
df['Discount_Rate'] = df['Discount %'] / 100.0

# Compute missing discount rate from prices
missing_dr = df['Discount_Rate'].isna()
valid = missing_dr & df['Original Price'].notna() & df['Sale Price'].notna() & (df['Original Price'] > 0)
df.loc[valid, 'Discount_Rate'] = 1 - df.loc[valid, 'Sale Price'] / df.loc[valid, 'Original Price']

# Fill missing Effective Price from Sale Price
df['Effective Price'] = df['Effective Price'].fillna(df['Sale Price'])

# Backfill nulls from latest known value per SKU (for Model Code mainly)
sku_col = 'Name'  # Use Name as unique identifier since no dedicated SKU column
for col_fill in ['Brand','Model Code','AC Type','Mode','Ton','Compressor','BTU']:
    if col_fill not in df.columns:
        continue
    lk = df.dropna(subset=[col_fill]).drop_duplicates(sku_col, keep='last').set_index(sku_col)[col_fill]
    m = df[col_fill].isna()
    df.loc[m, col_fill] = df.loc[m, sku_col].map(lk)

# Create a unique SKU-like key from Name (since Black Box has no SKU column)
...
# ── Read Rule sheet for stock status thresholds ────────────────────────────────
STOCK_RULES = [
    {'label': 'Out of Stock', 'min': None, 'max': 0},
    {'label': 'Critical', 'min': 0, 'max': 10},
    {'label': 'Low', 'min': 10, 'max': 20},
    {'label': 'High', 'min': 20, 'max': 50},
    {'label': 'OK', 'min': 50, 'max': None},
]
try:
    rule_df = pd.read_excel(INPUT_FILE, sheet_name='Rule')
    print(f"      Rule sheet loaded: {len(rule_df)} rules")
except:
    print("      Rule sheet not found, using defaults")
...
for _, r in df.iterrows():
    records.append({
        'd': str(r['date_only']) if pd.notna(r.get('Scraped At')) else None,
        'b': safe(r.get('Brand')),
        'n': str(r.get('Name',''))[:70] if pd.notna(r.get('Name')) else '',
        'm': str(r.get('Model Code','')) if pd.notna(r.get('Model Code')) else '',
        's': str(r.get('Name','')),  # SKU = Name (unique identifier)
        'c': safe(r.get('AC Type')),
        'h': safe(r.get('Mode')),
        't': safe(r.get('Ton')),
        'cp': safe(r.get('Compressor')),
        'sp': safe(r.get('Original Price')),
        'sl': safe(r.get('Sale Price')),
        'bp': safe(r.get('BP Price')),
        'dr': safe(r.get('Discount_Rate')),
        'fp': safe(r.get('Effective Price')),
        'fj': safe(r.get('Effective BP')),
        'ed': safe(r.get('Extra Disc %')),
        'fi': bool(r.get('Free Install')) if pd.notna(r.get('Free Install')) else False,
        'isar': safe(r.get('Install SAR')),
        'r10': bool(r.get('+10% Regular')) if pd.notna(r.get('+10% Regular')) else False,
        'bp10': bool(r.get('+10% BP Only')) if pd.notna(r.get('+10% BP Only')) else False,
        'se': str(r.get('Sale Ends','')) if pd.notna(r.get('Sale Ends')) else '',
        'ins': bool(r.get('In Stock')) if pd.notna(r.get('In Stock')) else False,
...
categories_list = sorted(df['AC Type'].dropna().unique().tolist())
compressors_list = sorted(df['Compressor'].dropna().unique().tolist())
cold_hc_list = sorted(df['Mode'].dropna().unique().tolist())
...
<title>Black Box AC Price Tracker</title>
...
<header class="bg-gradient-to-r from-navy-900 to-navy-700 text-white shadow-lg">
  <div class="max-w-[1600px] mx-auto px-6 py-4 flex flex-wrap items-center justify-between gap-4">
    <div><h1 class="text-xl font-bold tracking-wide">Black Box AC Price Tracker</h1>
...
<section id="sec-alert" class="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
  ...
  <div class="flex flex-wrap items-center gap-2 mb-2">
    <div class="flex rounded-lg overflow-hidden border border-gray-200">
      <button type="button" class="alert-tab active px-4 py-1 text-xs font-semibold bg-navy-800 text-white" data-tab="sale">Effective</button>
      <button type="button" class="alert-tab px-4 py-1 text-xs font-semibold bg-gray-50 text-gray-600" data-tab="jood">Eff. BP</button></div>
    <div class="flex rounded-lg overflow-hidden border border-gray-200">
...
<section id="sec-catKPI" class="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
  ...
  <div class="flex flex-wrap items-center gap-2 mb-3 pb-2 border-b border-gray-100">
    ...
    <select id="s4_pt" class="pt-sel"><option value="fp">Effective</option><option value="fj">Eff. BP</option><option value="sp">Standard</option></select>
...
<section id="sec-brand" class="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
  ...
  <div class="flex flex-wrap items-center gap-2 mb-3 pb-2 border-b border-gray-100">
    ...
    <select id="s5_pt" class="pt-sel"><option value="fp">Effective</option><option value="fj">Eff. BP</option><option value="sp">Standard</option></select>
    <div class="flex rounded-lg overflow-hidden border border-gray-200">
...
<section id="sec-trend" class="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
  ...
  <div class="flex flex-wrap items-center gap-2 mb-3 pb-2 border-b border-gray-100">
    ...
    <select id="s6_pt" class="pt-sel"><option value="fp">Effective</option><option value="fj">Eff. BP</option><option value="sp">Standard</option></select>
    <div class="flex rounded-lg overflow-hidden border border-gray-200">
...
<!-- SEC 7: Promo Analysis (Black Box specific) -->
<section id="sec-promo" class="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
  <h2 class="text-sm font-bold text-navy-800 border-b-2 border-navy-800 pb-2 mb-3">Promotion Analysis</h2>
  <div id="promoKpiGrid" class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5 mb-4"></div>
  <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
    <div style="height:280px"><canvas id="promoTypeDonut"></canvas></div>
    <div style="height:280px"><canvas id="installBar"></canvas></div>
    <div style="height:280px"><canvas id="saleEndsBar"></canvas></div>
  </div>
  <h3 class="text-xs font-bold text-gray-500 uppercase mt-2 mb-2">Sale Ending Soon</h3>
  <div class="tbl-wrap" style="max-height:260px;overflow-y:auto"><table id="tblSaleEnds"><thead></thead><tbody></tbody></table></div>
</section>
...
<section id="sec-sku" class="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
  ...
  <div class="flex flex-wrap items-center gap-2 mb-3 pb-2 border-b border-gray-100">
    ...
    <select id="s8_pt" class="pt-sel"><option value="fp">Effective</option><option value="fj">Eff. BP</option><option value="sp">Standard</option></select>
    <div class="flex rounded-lg overflow-hidden border border-gray-200">
...
<section id="sec-stock" class="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
  ...
  <div id="stockKpiGrid" class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-2.5 mb-4"></div>
...
<footer class="text-center py-3 text-[10px] text-gray-400">Black Box AC Price Tracking Dashboard v2 &middot; Generated GENERATED_AT</footer>
"""
...
const searchMatch=(r,q)=>!q||((r.b||'')+(r.s||'')+(r.m||'')+(r.n||'')+(r.c||'')+(r.cp||'')+(r.h||'')+(r.t||'')).toLowerCase().includes(q);

function stkStatus(qty){
  if(qty==null||qty<=0) return {label:'Out of Stock',cls:'stk-out'};
  if(qty<=10) return {label:'Critical',cls:'stk-critical'};
  if(qty<=20) return {label:'Low',cls:'stk-low'};
...
function makeFilters(prefix,cb){
  return {
    ...
    hc: new MS(document.getElementById(prefix+'_hc'),COLD_HC,'Mode',cb),
...
let gfDate,s9Date;
const ST={alertTab:'sale',alertDir:'all',s4q:'',s5q:'',s6q:'',s8q:'',s9q:'',skuStock:'all',stkDir:'all',s5agg:'avg',s6agg:'avg'};
...
function renderKPIs(lat,prev){
  ...
  const cards=[
    ...
    {v:fmtSAR(avgS),l:'Avg Eff. Price',c:'border-l-purple-500 bg-purple-50',vc:'text-purple-700'},
  ];
...
// ═══ SEC 2: ALERTS ═══════════════════════════════════════════════════════════
const AC=[{k:'b',l:'Brand'},{k:'s',l:'Product'},{k:'m',l:'Model'},{k:'c',l:'AC Type'},{k:'cp',l:'Compressor'},{k:'h',l:'Mode'},{k:'ton',l:'Ton'},{k:'prev',l:'Prev',f:fmtSAR},{k:'curr',l:'Curr',f:fmtSAR},{k:'chg',l:'Change',f:fmtChg},{k:'chgPct',l:'Chg%',f:fmtPctR}];
function renderAlerts(){
  ...
  Object.keys(lm).filter(s=>s in pm).forEach(s=>{
    ...
    rows.push({b:rn.b,s:rn.n,m:rn.m,c:rn.c,cp:rn.cp||'-',h:rn.h||'-',ton:rn.t!=null?rn.t.toFixed(1)+'T':'-',prev:po,curr:pn,chg,chgPct:po?chg/po*100:0,url:rn.url});
  });
...
function renderNewDisc(lat,prev){
  ...
  const card=(r,clr)=>`<div class="border border-${clr}-200 bg-${clr}-50 rounded-lg p-2.5 flex justify-between items-center"><div><span class="text-xs font-bold" style="color:${colorOf(r.b)}">${r.b}</span> <span class="text-[10px] text-gray-400">${r.m||''}</span><div class="text-[11px] mt-0.5 truncate max-w-[280px]">${r.url?`<a href="${r.url}" target="_blank" class="text-blue-600 hover:text-blue-800 hover:underline">${r.n}</a>`:r.n}</div><div class="text-[10px] text-gray-400">${r.c||''} &middot; ${r.t?r.t.toFixed(1)+'T':''} &middot; Stock: ${r.stk!=null?r.stk:'N/A'}</div></div><div class="text-sm font-bold text-gray-700">${fmtSAR(r.fp)} SAR</div></div>`;
...
// ═══ SEC 4: CATEGORY KPI ════════════════════════════════════════════════════
const CK=[{l:'Type',k:'type'},{l:'Segment',k:'label'},{l:'SKUs',k:'cnt'},{l:'Avg Price',k:'avgP',f:fmtSAR},{l:'Avg Std',k:'avgStd',f:fmtSAR},{l:'Avg Disc %',k:'avgDisc',f:fmtPctR},{l:'In Stock',k:'inStk'},{l:'LG Avg',k:'lgAvg',f:fmtSAR},{l:'LG Gap',k:'lgGap',f:fmtChg},{l:'LG Gap %',k:'lgGapPct',f:fmtPctR}];
const TYPE_BADGE={cat:'<span class="type-badge type-cat">AC Type</span>',comp:'<span class="type-badge type-comp">Compressor</span>',hc:'<span class="type-badge type-hc">Mode</span>',ton:'<span class="type-badge type-ton">Ton</span>'};
const CAT_ORDER=['Split','Window','Cassette','Concealed','Freestanding','Portable'];
const COMP_ORDER=['Inverter','Rotary','Energy Saver'];
const HC_ORDER=['Cold Only','Cold and Hot'];
...
function renderS4(){
  ...
  const ptLabel={fp:'Effective',fj:'Eff. BP',sp:'Standard'}[pk]||'Price';
  CK[3].l='Avg '+ptLabel;
  const rows=[];
    ...
    [...new Set(cD.map(r=>r.cp))].filter(x=>x!=null&&x!=='').sort().forEach(comp=>{const coD=cD.filter(r=>r.cp===comp);
      [...new Set(coD.map(r=>r.h))].filter(x=>x!=null&&x!=='').sort().forEach(hc=>{const hD=coD.filter(r=>r.h===hc);
        [...new Set(hD.map(r=>r.t))].filter(x=>x!=null&&!isNaN(x)).sort((a,b)=>a-b).forEach(t=>{const tD=hD.filter(r=>r.t===t);if(!tD.length)return;
          ...
          bList.forEach(b=>{const bD=tD.filter(r=>r.b===b&&r[pk]!=null);const v=bD.length?aggFn5(bD.map(r=>r[pk])):null;row[b]=v;if(v!=null)prices.push(v);});
...
function renderS6(){
  ...
  let datasets=[];
  const mkt=DATES.map(d=>{let dd=applyF(DATA.filter(r=>r.d===d),S6F);if(q)dd=dd.filter(r=>searchMatch(r,q));dd=dd.filter(r=>r[pk]!=null);return dd.length?aggFn6(dd.map(r=>r[pk])):null;});
  ...
  [...bSet].forEach(br=>{
    const vals=DATES.map(d=>{let dd=applyF(DATA.filter(r=>r.d===d&&r.b===br),S6F);if(q)dd=dd.filter(r=>searchMatch(r,q));dd=dd.filter(r=>r[pk]!=null);return dd.length?aggFn6(dd.map(r=>r[pk])):null;});
...
// ═══ SEC 7: PROMO ANALYSIS (Black Box specific) ═════════════════════════════
let promoTypeChart=null,installChart=null,saleEndsChart=null;
function renderPromo(lat){
  // ── KPI Cards ──
  const freeInstall=lat.filter(r=>r.fi===true);
  const extraDisc=lat.filter(r=>r.ed!=null&&r.ed>0);
  const reg10=lat.filter(r=>r.r10===true);
  const bp10=lat.filter(r=>r.bp10===true);
  const hasSaleEnd=lat.filter(r=>r.se&&r.se.trim());
  const hasBP=lat.filter(r=>r.bp!=null&&r.bp>0);

  const promoCards=[
    {v:freeInstall.length,l:'Free Install',c:'border-l-green-500 bg-green-50',vc:'text-green-600'},
    {v:extraDisc.length,l:'Extra 10% Off',c:'border-l-blue-500 bg-blue-50',vc:'text-blue-600'},
    {v:reg10.length,l:'+10% Regular',c:'border-l-amber-500 bg-amber-50',vc:'text-amber-700'},
    {v:bp10.length,l:'+10% BP Only',c:'border-l-purple-500 bg-purple-50',vc:'text-purple-600'},
    {v:hasSaleEnd.length,l:'With Sale End',c:'border-l-red-500 bg-red-50',vc:'text-red-600'},
    {v:hasBP.length,l:'Has BP Price',c:'border-l-teal-500 bg-teal-50',vc:'text-teal-600'},
  ];
  document.getElementById('promoKpiGrid').innerHTML=promoCards.map(c=>`<div class="rounded-lg border-l-4 ${c.c} p-3"><div class="text-xl font-bold ${c.vc}">${c.v}</div><div class="text-[10px] text-gray-500 mt-1 font-medium">${c.l}</div></div>`).join('');

  // ── Promo Type Distribution Donut ──
  const promoBreakdown={
    'Free Install Only':0,'Extra 10% Only':0,'+10% Regular':0,'+10% BP Only':0,'Multiple Promos':0,'No Promo':0
  };
  lat.forEach(r=>{
    const promos=[];
    if(r.fi)promos.push('fi');
    if(r.ed!=null&&r.ed>0)promos.push('ed');
    if(r.r10)promos.push('r10');
    if(r.bp10)promos.push('bp10');
    if(promos.length===0)promoBreakdown['No Promo']++;
    else if(promos.length>1)promoBreakdown['Multiple Promos']++;
    else if(promos[0]==='fi')promoBreakdown['Free Install Only']++;
    else if(promos[0]==='ed')promoBreakdown['Extra 10% Only']++;
    else if(promos[0]==='r10')promoBreakdown['+10% Regular']++;
    else if(promos[0]==='bp10')promoBreakdown['+10% BP Only']++;
  });
  const pl=Object.keys(promoBreakdown).filter(k=>promoBreakdown[k]>0);
  const pv=pl.map(k=>promoBreakdown[k]);
  const pcol=['#16a34a','#2563eb','#d97706','#7c3aed','#ea580c','#94a3b8'];
  const c1=document.getElementById('promoTypeDonut').getContext('2d');if(promoTypeChart)promoTypeChart.destroy();
  promoTypeChart=new Chart(c1,{type:'doughnut',data:{labels:pl,datasets:[{data:pv,backgroundColor:pcol.slice(0,pl.length)}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom',labels:{font:{size:9}}},title:{display:true,text:'Promo Type Distribution',font:{size:12}},datalabels:{display:true,color:'#fff',font:{size:11,weight:'bold'},formatter:(v,ctx)=>{const t=ctx.dataset.data.reduce((a,b)=>a+b,0);return t?(v/t*100).toFixed(0)+'%':'';}}}}});

  // ── Free Install by Brand Bar ──
  const instBrand={};lat.forEach(r=>{
    if(!instBrand[r.b])instBrand[r.b]={free:0,paid:0,total:0};
    instBrand[r.b].total++;
    if(r.fi)instBrand[r.b].free++;else instBrand[r.b].paid++;
  });
  const iBrands=Object.keys(instBrand).sort((a,b)=>instBrand[b].free-instBrand[a].free);
  const c2=document.getElementById('installBar').getContext('2d');if(installChart)installChart.destroy();
  installChart=new Chart(c2,{type:'bar',data:{labels:iBrands,datasets:[
    {label:'Free Install',data:iBrands.map(b=>instBrand[b].free),backgroundColor:'#16a34a'},
    {label:'Paid/None',data:iBrands.map(b=>instBrand[b].paid),backgroundColor:'#e5e7eb'}
  ]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom',labels:{font:{size:9}}},title:{display:true,text:'Installation by Brand',font:{size:12}}},scales:{x:{ticks:{font:{size:9},maxRotation:45},stacked:true},y:{stacked:true,ticks:{stepSize:5}}}}});

  // ── Sale Ends Timeline Bar ──
  const seCount={};hasSaleEnd.forEach(r=>{const d=r.se.trim();seCount[d]=(seCount[d]||0)+1;});
  const seDates=Object.keys(seCount).sort();
  const c3=document.getElementById('saleEndsBar').getContext('2d');if(saleEndsChart)saleEndsChart.destroy();
  saleEndsChart=new Chart(c3,{type:'bar',data:{labels:seDates,datasets:[{label:'SKUs Ending',data:seDates.map(d=>seCount[d]),backgroundColor:seDates.map(d=>{
    // Color coding: past/today = red, within 7 days = amber, later = blue
    const diff=(new Date(d)-new Date(LATEST_DATE))/(86400000);
    return diff<=0?'#dc2626':diff<=7?'#d97706':'#2563eb';
  })}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},title:{display:true,text:'Sale End Dates',font:{size:12}},datalabels:{display:true,anchor:'end',align:'top',color:'#1e3a5f',font:{size:10,weight:'bold'}}},scales:{x:{ticks:{font:{size:9},maxRotation:45}},y:{ticks:{stepSize:5}}}}});

  // ── Sale Ending Soon Table ──
  const soonRows=hasSaleEnd.map(r=>{
    const diff=Math.ceil((new Date(r.se.trim())-new Date(LATEST_DATE))/86400000);
    return{b:r.b,n:r.n,m:r.m,c:r.c,fp:r.fp,se:r.se.trim(),daysLeft:diff,stk:r.stk,url:r.url};
  }).sort((a,b)=>a.daysLeft-b.daysLeft);
  const seCols=[{k:'b',l:'Brand'},{k:'n',l:'Product'},{k:'m',l:'Model'},{k:'c',l:'AC Type'},{k:'fp',l:'Eff. Price',f:fmtSAR},{k:'se',l:'Sale Ends'},{k:'daysLeft',l:'Days Left'},{k:'stk',l:'Stock'}];
  const seTbl=document.getElementById('tblSaleEnds');
  seTbl.querySelector('thead').innerHTML='<tr>'+seCols.map((c,i)=>`<th onclick="sortTbl('tblSaleEnds',${i})">${c.l}</th>`).join('')+'</tr>';
  seTbl.querySelector('tbody').innerHTML=soonRows.length?soonRows.map(r=>'<tr>'+seCols.map(c=>{
    let v=c.f?c.f(r[c.k]):(r[c.k]!=null?r[c.k]:'-');let cls='';
    if(c.k==='daysLeft'){cls=r.daysLeft<=0?'up-cell':r.daysLeft<=7?'text-amber-600 font-semibold':'text-blue-600';v=r.daysLeft<=0?'Ended':r.daysLeft+'d';}
    if(c.k==='b')v=`<span style="color:${colorOf(r.b)};font-weight:600">${v}</span>`;
    if(c.k==='n'&&r.url)v=`<a href="${r.url}" target="_blank" class="text-blue-600 hover:text-blue-800 hover:underline">${v}</a>`;
    if(c.k==='stk'){const sv=r.stk;if(sv!=null&&sv>0)v=`<span class="text-green-600 font-semibold">${sv}</span>`;else v=`<span class="text-red-500 font-semibold">${sv===0?'0':'N/A'}</span>`;}
    return`<td class="${cls}">${v}</td>`;
  }).join('')+'</tr>').join(''):'<tr><td colspan="8" class="text-center text-gray-400 py-4">No sale end dates</td></tr>';
}

// ═══ SEC 8: FULL SKU ════════════════════════════════════════════════════════
const SC=[{k:'b',l:'Brand'},{k:'n',l:'Product'},{k:'m',l:'Model'},{k:'c',l:'AC Type'},{k:'cp',l:'Compressor'},{k:'h',l:'Mode'},{k:'ton',l:'Ton'},{k:'sp',l:'Std Price',f:fmtSAR},{k:'sl',l:'Sale Price',f:fmtSAR},{k:'bp',l:'BP Price',f:fmtSAR},{k:'dr',l:'Disc %',f:fmtPct},{k:'fp',l:'Eff. Price',f:fmtSAR},{k:'fj',l:'Eff. BP',f:fmtSAR},{k:'prev',l:'Prev Price',f:fmtSAR},{k:'chg',l:'Change',f:fmtChg},{k:'chgPct',l:'Chg %',f:fmtPctR},{k:'stkBadge',l:'Stock Status'},{k:'stk',l:'Stock Qty'},{k:'promo',l:'Promo'}];
let _skuData=[];
...
function renderS8(){
  ...
  let rows=lat.map(r=>{
    ...
    // Build promo tags
    const tags=[];
    if(r.fi)tags.push('Install');
    if(r.ed!=null&&r.ed>0)tags.push('Extra10%');
    if(r.r10)tags.push('+10%Reg');
    if(r.bp10)tags.push('+10%BP');
    return{...r,ton:r.t!=null?r.t.toFixed(1)+'T':'-',prev:prevP,chg,chgPct,promo:tags.join(', ')||'-',stkBadge:r.stk,link:r.url};
  });
  ...
  tbl.querySelector('tbody').innerHTML=rows.length?rows.map(r=>'<tr>'+SC.map(c=>{
    ...
    if(c.k==='b')v=`<span style="color:${colorOf(r.b)};font-weight:600">${v}</span>`;
    if(c.k==='n')v=r.url?`<a href="${r.url}" target="_blank" class="text-blue-600 hover:text-blue-800 hover:underline cursor-pointer" title="Open product page">${v}</a>`:v;
    if(c.k==='stkBadge'){const st=stkStatus(r.stk);v=`<span class="stk-badge ${st.cls}">${st.label}</span>`;}
    ...
    if(c.k==='promo'&&r.promo!=='-')v=r.promo.split(', ').map(t=>`<span class="inline-block px-1 py-0.5 bg-blue-100 text-blue-700 rounded text-[9px] font-semibold mr-0.5">${t}</span>`).join('');
    return`<td class="${cls}">${v}</td>`;
  }).join('')+'</tr>').join(''):'<tr><td colspan="20" class="text-center text-gray-400 py-6">No data</td></tr>';
}
...
function downloadExcel(){
  ...
  const xlData=rows.map(r=>({
    Brand:r.b,Product:r.n,Model:r.m,'AC Type':r.c,Compressor:r.cp,Mode:r.h,Ton:r.ton,
    'Std Price':r.sp,'Sale Price':r.sl,'BP Price':r.bp,'Disc %':r.dr!=null?Math.round(r.dr*1000)/10:null,
    'Eff Price':r.fp,'Eff BP':r.fj,'Prev Price':r.prev,'Change':r.chg,'Change %':r.chgPct,
    'Stock Status':stkStatus(r.stk).label,'Stock Qty':r.stk,Promo:r.promo,URL:r.url
  }));
  const ws=XLSX.utils.json_to_sheet(xlData);
  const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,ws,'SKU Data');
  XLSX.writeFile(wb,'BlackBox_AC_SKUs_'+cur+'.xlsx');
}
...
function renderStock(){
  ...
  // ── 9-1: KPI Cards (5-tier stock status) ───────────────────────────────────
  const inStock=latD.filter(r=>r.stk!=null&&r.stk>0);
  const outStock=latD.filter(r=>r.stk==null||r.stk===0);
  const critical=latD.filter(r=>r.stk!=null&&r.stk>0&&r.stk<=10);
  const low=latD.filter(r=>r.stk!=null&&r.stk>10&&r.stk<=20);
  ...
  const stkCards=[
    ...
    {v:critical.length,l:'Critical (<=10)',c:'border-l-orange-500 bg-orange-50',vc:'text-orange-600'},
    ...
    {v:topOutLabel,l:'Most Stock-outs',c:'border-l-purple-500 bg-purple-50',vc:'text-purple-600'},
  ];
  ...
  allSkus.forEach(s=>{
    ...
    stkRows.push({b:ref.b,s,n:ref.n,c:ref.c||'-',ton:ref.t!=null?ref.t.toFixed(1)+'T':'-',prevStk,curStk,chg,chgPct,isNewOut,curStatus:stkStatus(curStk).label,url:ref.url});
  });
  ...
  const saCols=[{k:'b',l:'Brand'},{k:'n',l:'Product'},{k:'c',l:'AC Type'},{k:'ton',l:'Ton'},{k:'prevStk',l:fromDate},{k:'curStk',l:toDate},{k:'chg',l:'Change'},{k:'chgPct',l:'Chg%'},{k:'curStatus',l:'Status'}];
  ...
  stbl.querySelector('tbody').innerHTML=stkRows.length?stkRows.map(r=>'<tr>'+saCols.map(c=>{
    ...
    if(c.k==='curStatus'){const st=stkStatus(r.curStk);v=`<span class="stk-badge ${st.cls}">${st.label}</span>`;}
    return`<td class="${cls}">${v}</td>`;
  }).join('')+'</tr>').join(''):'<tr><td colspan="9" class="text-center text-gray-400 py-6">No stock changes between selected dates</td></tr>';

  // ── 9-4: Stock Distribution Charts ─────────────────────────────────────────
  ...
  stockCatChartObj=new Chart(ctx2,{type:'doughnut',data:{labels:cl,datasets:[{data:cv,backgroundColor:catCols.slice(0,cl.length)}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom',labels:{font:{size:10}}},title:{display:true,text:'Stock Quantity by AC Type ('+toDate+')',font:{size:12}},datalabels:{display:true,color:'#fff',font:{size:11,weight:'bold'},formatter:(v,ctx2b)=>{const t=ctx2b.dataset.data.reduce((a,b)=>a+b,0);return t?(v/t*100).toFixed(1)+'%':'';}}}}});
}
...
DEPLOY_DIR = os.path.join(os.path.expanduser("~"), "tmp_deploy_blackbox")
...
if os.path.isdir(os.path.join(DEPLOY_DIR, ".git")):
    ...
    print(f"\nNo deploy repo found at {DEPLOY_DIR}")
    print("To set up GitHub Pages deployment, create the repo first.")
...
CLOUDFLARE_DEST = os.path.join(SHAKER_DIR, "docs", "dashboards", "blackbox-price")
if os.path.exists(os.path.join(SHAKER_DIR, ".git")):
    ...
    try:
        subprocess.run(["git", "add", "docs/dashboards/blackbox-price/index.html"],
                       cwd=SHAKER_DIR, check=True, capture_output=True)
        result = subprocess.run(
            ["git", "commit", "-m",
             f"Update Black Box Price dashboard ({generated_at})"],
