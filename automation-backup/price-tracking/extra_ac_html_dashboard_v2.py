...
SPLIT_KEYWORDS = ['SPLIT', 'SPILT', 'WALL MOUNT', 'WALL-MOUNT']
...
# null 필드 폴백 처리 (applyF 필터에서 걸러지지 않도록)
null_brand = df['Brand'].isna().sum()
null_comp = df['Compressor_Type'].isna().sum()
if null_brand > 0:
    print(f"      ⚠️ {null_brand} rows with null Brand → 'Unknown'")
    df['Brand'] = df['Brand'].fillna('Unknown')
if null_comp > 0:
    print(f"      ⚠️ {null_comp} rows with null Compressor → 'Other'")
    df['Compressor_Type'] = df['Compressor_Type'].fillna('Other')
...
for _, r in df.iterrows():
    records.append({
        ...
        'url': f'https://www.extra.com/en-sa/p/{r.get("SKU","")}' if pd.notna(r.get('SKU')) else '',
    })
...
sku_date_map  = df.groupby('SKU')['date_only'].apply(lambda s: set(d for d in s if pd.notna(d))).to_dict()
...
for sku, dates_set in sku_date_map.items():
    ...
    if last_d == latest_d:
        ...
        row = df[(df['SKU']==sku) & (df['date_only']==last_d)].iloc[0]
        disc_records.append({
            ...
            'url': f'https://www.extra.com/en-sa/p/{sku}' if sku else '',
        })
...
const CAT_ORDER=['Free Standing Air Conditioner','Split Air Conditioner','Window Air Conditioner','Portable Air Conditioner'];
