#!/usr/bin/env python3
"""
Al Khunaizan AC Price Tracker - 통합 실행 스크립트 (Run All)
3개 스크립트를 순차 실행합니다:
  1. AC_Scraper_AlKhunaizan_v8_2.py    - 가격 스크래핑
  2. alkhunaizan_dashboard_builder.py   - 대시보드 빌드 (Excel)
  3. alkhunaizan_ac_html_dashboard_v2.py - HTML 대시보드 생성
...
SCRIPTS = [
    {
        "file": "AC_Scraper_AlKhunaizan_v8_2.py",
        "name": "가격 스크래핑",
        "icon": "🌐",
    },
    {
        "file": "alkhunaizan_dashboard_builder.py",
        "name": "대시보드 빌드 (Excel)",
        "icon": "📊",
    },
    {
        "file": "alkhunaizan_ac_html_dashboard_v2.py",
        "name": "HTML 대시보드 생성",
        "icon": "🖥️",
    },
]
...
def run_script(script_info, step, total):
    ...
    try:
        result = subprocess.run(
            [PYTHON_CMD, "-X", "utf8", filepath],
            cwd=CURRENT_DIR,
            stdin=subprocess.DEVNULL,   # 자식 스크립트의 input() 대기 방지
...
def main():
    ...
    print("=" * 60)
    print("🚀 Al Khunaizan AC Price Tracker - 통합 실행 (Run All)")
    ...
    for i, script_info in enumerate(SCRIPTS, 1):
        ...
        if i == 1:
            try:
                ...
                run_with_retry(
                    {'name': 'Al Khunizan', 'master': _SCRIPT_DIR / 'AlKhunaizan_AC_Prices Tracking_Master.xlsx',
                     'sku_col': 'SKU', 'date_col': 'Scraped_At', 'sheet_name': 'Products_DB'},
