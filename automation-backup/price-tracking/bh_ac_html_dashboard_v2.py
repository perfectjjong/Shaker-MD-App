...
def main():
    ...
    # ── RT SKU 4-way Status Classification (BH B안: Retail만 적용) ──
    # New / Reactive / Temp OOS / Discontinued (BH RT는 Model Code 'm' 기준)
    TEMP_OOS_THRESHOLD = 14
    REACTIVE_GAP_MIN = 2

    rt_all_dates = sorted(set(r['d'] for r in rt_data if r.get('d')))
    rt_sku_status = {}
    rt_disc_records = []
    if rt_all_dates:
        rt_latest_d = rt_all_dates[-1]
        rt_model_dates = {}
        for r in rt_data:
            if r.get('m') and r.get('d'):
                rt_model_dates.setdefault(r['m'], set()).add(r['d'])

        for m_code, dates_set in rt_model_dates.items():
            first_d = min(dates_set)
            last_d = max(dates_set)

            if last_d == rt_latest_d:
                if first_d == rt_latest_d:
                    rt_sku_status[m_code] = {'st':'new','ab':0,'ls':last_d,'fs':first_d,'gb':0}
                else:
                    idx = rt_all_dates.index(rt_latest_d)
                    gap = 0
                    for pd_ in reversed(rt_all_dates[:idx]):
                        if pd_ not in dates_set:
                            gap += 1
                        else:
                            break
                    if gap >= REACTIVE_GAP_MIN:
                        rt_sku_status[m_code] = {'st':'reactive','ab':0,'ls':last_d,'fs':first_d,'gb':gap}
            else:
                absent_days = sum(1 for d in rt_all_dates if d > last_d)
                st = 'disc' if absent_days >= TEMP_OOS_THRESHOLD else 'temp_disc'
                rt_sku_status[m_code] = {'st':st,'ab':absent_days,'ls':last_d,'fs':first_d,'gb':0}
                last_rec = next((r for r in rt_data if r.get('m')==m_code and r.get('d')==last_d), {})
                rt_disc_records.append({
                    's': m_code, 'st': st, 'ab': absent_days, 'ls': last_d,
                    'b': last_rec.get('b','') or '',
                    'n': last_rec.get('n','') or '',
                    'm': m_code,
                    'c': last_rec.get('c','') or '',
                    'h': last_rec.get('tp','') or '',
                    't': last_rec.get('t'),
                    'fp': last_rec.get('cp'),
                    'url': last_rec.get('url','') or '',
                })
...
/* Light mode badges */
.stk-ok{background:#dcfce7;color:#15803d;padding:1px 6px;border-radius:4px;font-size:10px;font-weight:600}
.stk-high{background:#dbeafe;color:#1d4ed8;padding:1px 6px;border-radius:4px;font-size:10px;font-weight:600}
.stk-low{background:#fef3c7;color:#b45309;padding:1px 6px;border-radius:4px;font-size:10px;font-weight:600}
.stk-critical{background:#fee2e2;color:#b91c1c;padding:1px 6px;border-radius:4px;font-size:10px;font-weight:600}
.stk-out{background:#fce7f3;color:#9f1239;padding:1px 6px;border-radius:4px;font-size:10px;font-weight:600}
.cb-yes{background:#dcfce7;color:#15803d;padding:1px 6px;border-radius:4px;font-size:10px;font-weight:600}
.cb-no{background:#f1f5f9;color:#64748b;padding:1px 6px;border-radius:4px;font-size:10px;font-weight:600}
.fi-yes{background:#dbeafe;color:#1d4ed8;padding:1px 6px;border-radius:4px;font-size:10px;font-weight:600}
.fi-riyadh{background:#fef3c7;color:#b45309;padding:1px 6px;border-radius:4px;font-size:10px;font-weight:600}
.fi-no{background:#f1f5f9;color:#64748b;padding:1px 6px;border-radius:4px;font-size:10px;font-weight:600}
.promo-badge{background:#fef3c7;color:#b45309;padding:1px 6px;border-radius:4px;font-size:10px;font-weight:600}
/* RT SKU Status Tracker (4-way tabs + cards) */
.rt-sku-tab{padding:8px 14px;font-size:12px;font-weight:600;border-bottom:2px solid transparent;color:#6b7280;cursor:pointer;background:none;border-left:none;border-right:none;border-top:none;transition:all .15s}
.rt-sku-tab:hover{color:#1F4E79;background:#f8fafc}
.rt-sku-tab.active[data-tab="new"]{color:#15803d;border-bottom-color:#16a34a}
.rt-sku-tab.active[data-tab="reactive"]{color:#a16207;border-bottom-color:#eab308}
.rt-sku-tab.active[data-tab="temp_disc"]{color:#c2410c;border-bottom-color:#f97316}
.rt-sku-tab.active[data-tab="disc"]{color:#b91c1c;border-bottom-color:#ef4444}
.rt-sku-tab span{color:#9ca3af;font-weight:500;margin-left:3px}
.sku-card-new{border-color:#bbf7d0;background:#f0fdf4}
.sku-card-reactive{border-color:#fef08a;background:#fefce8}
.sku-card-temp{border-color:#fed7aa;background:#fff7ed}
.sku-card-disc{border-color:#fecaca;background:#fef2f2}
.sku-meta-new{font-size:10px;color:#15803d;font-weight:600}
.sku-meta-reactive{font-size:10px;color:#a16207;font-weight:600}
.sku-meta-temp{font-size:10px;color:#c2410c;font-weight:600}
.sku-meta-disc{font-size:10px;color:#b91c1c;font-weight:600}
</style></head>
"""
...
<div id="tab_wholesale" class="tab-panel">
  ...
  <div class="bg-white rounded-xl border p-4 mb-4 shadow-sm">
    <div class="flex items-center justify-between mb-3 flex-wrap gap-2">
      <h2 class="text-base font-bold text-navy-800 border-b-2 border-navy-800 pb-2">Wholesale Price Alerts <span id="ws_alert_compare" class="text-xs font-normal text-gray-400"></span></h2>
      <div class="flex items-center gap-2">
...
<div id="tab_retail" class="tab-panel">
  ...
  <div class="bg-white rounded-xl border p-4 mb-4 shadow-sm">
    <div class="flex items-center justify-between mb-3 flex-wrap gap-2">
      <h2 class="text-base font-bold text-navy-800 border-b-2 border-navy-800 pb-2">Retail Price Alerts <span id="rt_alert_compare" class="text-xs font-normal text-gray-400"></span></h2>
      <div class="flex items-center gap-2">
  ...
  <!-- RT-S3: SKU Status Tracker (4-way) -->
  <div class="bg-white rounded-xl border p-4 mb-4 shadow-sm">
    <h2 class="text-base font-bold text-navy-800 mb-1">SKU Status Tracker</h2>
    <p class="text-[11px] text-gray-500 mb-3">Retail SKU 4-way classification — New / Reactive / Temp OOS / Discontinued</p>
    <div class="flex gap-1 mb-3 border-b">
      <button class="rt-sku-tab active" data-tab="new" onclick="setRtSkuTab('new')">New <span id="rt_sku_new_cnt">(0)</span></button>
      <button class="rt-sku-tab" data-tab="reactive" onclick="setRtSkuTab('reactive')">Reactive <span id="rt_sku_reactive_cnt">(0)</span></button>
      <button class="rt-sku-tab" data-tab="temp_disc" onclick="setRtSkuTab('temp_disc')">Temp OOS <span id="rt_sku_temp_disc_cnt">(0)</span></button>
      <button class="rt-sku-tab" data-tab="disc" onclick="setRtSkuTab('disc')">Discontinued <span id="rt_sku_disc_cnt">(0)</span></button>
    </div>
    <p id="rt_sku_desc" class="text-[11px] text-gray-600 mb-3"></p>
    <div id="rtSkuStatusGrid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 max-h-[500px] overflow-y-auto"></div>
  </div>
...
const RT_SKU_STATUS={json.dumps(rt_sku_status,ensure_ascii=False)};
const RT_DISC_RECORDS={json.dumps(rt_disc_records,ensure_ascii=False)};
...
function renderWsKpis(){
  ...
  document.getElementById('ws_alert_compare').textContent=prev?'(vs '+prev+')':'';
...
function renderRtKpis(){
  ...
  document.getElementById('rt_alert_compare').textContent=prev?'(vs '+prev+')':'';
...
function setRtSkuTab(t){
  RT_SKU_TAB=t;
  document.querySelectorAll('.rt-sku-tab').forEach(b=>{
    b.classList.toggle('active',b.dataset.tab===t);
  });
  renderRtSkuStatus();
}
// Build a model→latest record lookup for RT (one-time, since RT data is static)
const RT_MODEL_INFO={};
RT.forEach(r=>{
  if(!r.m)return;
  const cur=RT_MODEL_INFO[r.m];
  if(!cur||r.d>cur._d)RT_MODEL_INFO[r.m]={b:r.b,n:r.n,c:r.c,cp:r.cp,_d:r.d};
});
function renderRtSkuStatus(){
  const buckets={new:[],reactive:[],temp_disc:[],disc:[]};
  Object.entries(RT_SKU_STATUS).forEach(([m,v])=>{
    if(buckets[v.st])buckets[v.st].push({m,...v});
  });
  document.getElementById('rt_sku_new_cnt').textContent='('+buckets.new.length+')';
  document.getElementById('rt_sku_reactive_cnt').textContent='('+buckets.reactive.length+')';
  document.getElementById('rt_sku_temp_disc_cnt').textContent='('+buckets.temp_disc.length+')';
  document.getElementById('rt_sku_disc_cnt').textContent='('+buckets.disc.length+')';
  document.getElementById('rt_sku_desc').textContent=RT_SKU_DESC[RT_SKU_TAB]||'';
  const list=buckets[RT_SKU_TAB]||[];
  const cardCls={new:'sku-card-new',reactive:'sku-card-reactive',temp_disc:'sku-card-temp',disc:'sku-card-disc'}[RT_SKU_TAB]||'';
  const card=v=>{
    const info=RT_MODEL_INFO[v.m]||{};
    const meta=RT_SKU_TAB==='reactive'?`<span class="sku-meta-reactive">Gap ${v.gb}d</span>`:
                RT_SKU_TAB==='temp_disc'?`<span class="sku-meta-temp">Absent ${v.ab}d</span>`:
                RT_SKU_TAB==='disc'?`<span class="sku-meta-disc">Absent ${v.ab}d · last ${v.ls}</span>`:
                `<span class="sku-meta-new">Since ${v.fs}</span>`;
    const brand=info.b||'-';
    const name=(info.n||'-').replace(/"/g,'&quot;');
    return `<div class="border rounded-lg p-2 ${cardCls}"><div class="flex items-center justify-between gap-2"><span style="color:${colorOf(brand)};font-weight:700" class="text-xs">${brand}</span>${meta}</div><div class="text-[10px] text-gray-700 mt-1 truncate" title="${name}">${name}</div><div class="flex justify-between mt-1 text-[10px]"><span class="text-gray-500">${info.c||'-'} · ${v.m}</span><span class="font-semibold">${info.cp!=null?'SAR '+fmtSAR(info.cp):'-'}</span></div></div>`;
  };
  document.getElementById('rtSkuStatusGrid').innerHTML=list.length?list.map(card).join(''):`<div class="text-gray-400 text-xs py-4 text-center col-span-full">No ${RT_SKU_TAB.replace('_',' ')} SKUs</div>`;
}
...
function renderRT(){renderRtKpis();renderRtAlerts();renderRtSkuStatus();renderRtCatKpi();renderRtBrandBar();renderRtTrend();renderRtFull();}
