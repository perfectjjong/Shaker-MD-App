...
def main():
    ...
    try:
        ...
        _SD = _P(__file__).parent
        sys.path.insert(0, str(_SD.parent))
        from scrape_quality_check import run_with_retry
        def _retry_almanea():
            try:
                m, s = _load_module('almanea_ac_v3.py')
                builtins.input = lambda *a, **kw: None
                s.loader.exec_module(m); m.main()
                return True
            except Exception as ex:
                print(f"  [Almanea Retry 실패] {ex}")
                return False
        run_with_retry(
            {'name': 'Al Manea', 'master': _SD / 'Almanea_AC_Price_Tracking_Master.xlsx',
             'sku_col': 'SKU', 'date_col': 'Scraped_At', 'sheet_name': 'Products_DB'},
            scraper_runner=_retry_almanea,
        )
    except Exception as e:
