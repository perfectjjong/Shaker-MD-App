#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""OR Main 8채널 — 판매(PSI) + 손익(GPC) 통합 데이터 빌더
...
OUT = os.path.join(D, "or-main-psi-gpc", "data.js")
...
# OR 5채널 — b2c-monthly-psi psi_data.js 의 meta.groups.OR 과 동일 축
OR_MAIN = ["eXtra", "Al Manea", "SWS", "Black Box", "Al Khunizan"]
...
def main():
    ...
    psi = _js(os.path.join(D, "b2c-monthly-psi", "psi_data.js"))
    for y in YEARS:
        for ch in OR_MAIN:
    ...
    for r in grows:
        ...
        if y not in YEARS or not (1 <= r["m"] <= 8):
            continue
        if r["ch"] != "OR" or r["ac"] not in OR_MAIN:
            continue
    ...
    payload = {
        "meta": {
            ...
            "channels": OR_MAIN, "months": MONTHS, "years": YEARS,
            "cats": cats, "ladder": LADDER,
            "oudAsOf": {m: asof.get(m) for m in MONTHS},
            "fin": _fin(),
            "notes": {
                "psi": "b2c-monthly-psi by_ch_cat 실측 월마감 (수량, 대) — OR·IR 공통 정본",
    ...
    with open(OUT, "w", encoding="utf-8") as f:
        f.write("// OR Main 8채널 판매+손익 통합 (build_or_main_psi_gpc.py 자동생성 — 직접 수정 금지)\n")
