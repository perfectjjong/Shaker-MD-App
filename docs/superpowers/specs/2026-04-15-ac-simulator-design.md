...
| 채널 유형 | LG 판매량 | 경쟁사 판매량 | 가격 데이터 | 특이사항 |
|---------|---------|------------|----------|---------|
| eXtra (OR) | ✅ 일별, 2024~ | ✅ 전체 브랜드 | ✅ VAT 제외 실판매가 | 탄력성 도출 유일한 채널 |
| SWS/Black Box/Al Manea (OR) | ✅ 주별, LG만 | ❌ | ✅ Price tracking (2026.3~) | 6주 데이터만 |
| IR 9개 채널 | ✅ 주별, LG만 | ❌ | ✅ Price tracking (2026.3~) | 6주 데이터만 |

**핵심 전략:** eXtra는 가격 탄력성 분석의 유일한 "실험실". eXtra에서 도출한 탄력성을 다른 채널에 transfer.
...
generate_forecast_data.py  [NEW]
  입력: extra-sellout/data.json + 13채널 weekly sell-out 데이터
  처리:
    - LG 판매량 집계 (channel × sub_family × type × size × week)
    - item_master.json으로 LG 모델 코드 역추적 (description 파싱)
    - ETS(지수평활) 또는 Prophet으로 Q2-Q4 주간 예측
    - 시즌 팩터 자동 추출 (Ramadan, 여름 피크)
  ...
  "by_channel": {
    "extra": {
      "APQ55GT3E4": {
        ...
        "season_peak": "Q3",
        "data_quality": "ok"  // "ok", "limited" (< 26주), "insufficient"
      }
    }
  }
}
